...
BYTE* FindPattern(BYTE* dwAddress, __int64 dwSize, BYTE* pbSig, const char* szMask)
{
	...
	for (__int64 i = (__int64)dwAddress; i < length; i += 4) // might run faster with 8 bytes but I am too lazy
	{
...
//static std::unordered_map<std::string, Signature> s_Signatures;
...
constexpr const char* OffsetToString(Offset offset)
{
	...
	//case Offset::ExposureFixed:				return "ExposureFixed";
	//case Offset::ExposureMode:				return "ExposureMode";
...
bool Offsets::ScanOffsets()
{
	...
	//s_Signatures[Offset::ExposureFixed]				= Signature("E8 ?? ?? ?? ?? C4 C1 7A 11 45 44 0F B6 05 [ ?? ?? ?? ?? ]", -0x15);
	//s_Signatures[Offset::ExposureMode]				= Signature("0F 8F 17 17 00 00 0F B6 05 [ ?? ?? ?? ?? ]", -0x15);
		...
		for (auto& sig : s_Signatures)
		{
			...
			if (s_checkOffsetCount)
			{
				...
				while (nextResult)
				{
					...
					nextResult = (__int64)(__int64)FindPattern((BYTE*)startAddress, newSize, sig.Pattern, sig.Mask.c_str());
				}
			}
...
/*long long Offsets::GetOffset(std::string const& name)
{
	auto result = s_Signatures.find(name);
	if (result != s_Signatures.end())
	{
...
constexpr long long GetOffset(Offset eOffset)
{
